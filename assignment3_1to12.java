...
public class assignment3_1to12 {
	...
	//1.STARTING SMALL
		...
		//CALLING METHODS
		iterating(colors); updating(colors); removing(colors); searching(colors); 
		sorting(colors); reversing(colors); copying(colors); comparing(colors); 
		appending(colors); looping(colors); converting(colors);
	
	}

	//2.ITERATING : WRITE A JAVA METHOD TO ITERATE THROUGH ALL ELEMENTS IN THE PREVIOUS ARRAYLIST.
	...
	//3.UPDATING : WRITE A METHOD TO CHANGE THE FIFTH ELEMENT OF YOUR ARRAYLIST TO "Magenta"
	...
	//5.SEARCHING : WRITE A METHOD TO SEARCH YOUR ARRAYLIST FOR A SPECIFIED COLOR.
		private static void searching(ArrayList<String> list) {
			System.out.println("\nQUESTION 5 -----------------------");
			String colorToFind = "Magenta";
			if (list.contains(colorToFind)) {
	...
	//8.COPYING : WRITE A METHOD THAT WILL COPY THE CONTENTS OF ONE ARRAYLIST TO ANOTHER.
		private static void copying(ArrayList<String> list) {
			System.out.println("\nQUESTION 8 -----------------------");
		
		}
		
	//9.COMPARING : WRITE A METHOD THAT WILL COMPARE TWO ARRAYLISTS, PRINTING OUT WHICH ELEMENTS THEY HAVE IN COMMON.
	...
	//12.CONVERTING : WRITE A METHOD THAT WILL CONVERT A REGULAR ARRAY TO AN ARRAYLIST.
